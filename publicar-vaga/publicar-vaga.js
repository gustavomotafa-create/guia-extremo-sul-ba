...
form.addEventListener("submit", async (event) => {
  ...
  if (!currentUser) {
    window.location.href = "../login/?next=../publicar-vaga/";
    return;
  }
  ...
  try {
    ...
    if (result.requiresPayment) {
      showStatus(
        "Pagamento necessário para publicar mais oportunidades hoje.",
        "Você já usou as 2 publicações gratuitas. Pague R$ 5,00 para liberar mais 2 publicações extras hoje.",
    ...
    form.reset();
    showStatus("Oportunidade enviada para aprovação.", "A publicação entrou como pendente e será analisada pelo admin do Girassol Vagas.");
  } catch (error) {
    showStatus("Não foi possível enviar a oportunidade.", error.message || "Tente novamente em alguns instantes.");
  }
});
